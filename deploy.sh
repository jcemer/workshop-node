...
npm install . && cake build && git checkout gh-pages && rm .gitignore && rm -rf node_modules && git add . && git commit -m "Regenerate" && git push origin gh-pages && git checkout master